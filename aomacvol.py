...
class TradingBacktester:
    ...
    def calculate_signals(self, df: pd.DataFrame, window: int = 5) -> pd.Series:
        ...
        # Enhanced buy conditions (more relaxed)
        ao_buy = (df['AO'] < 0) & (df['AO'] > df['AO_MA'])  # Original AO condition
        macd_buy = df['MACD'] > df['MACD_Signal']  # Trend confirmation
        volume_buy = df['OBV'] > df['OBV_MA']  # Volume confirmation
        
        # Buy signal requires AO condition plus either MACD trend or volume confirmation
        buy_signal = (
            ao_buy & 
            (
                (macd_buy & volume_buy) |  # Both MACD and volume are positive
                (macd_buy & price_trend) |  # MACD and price trend are positive
                (volume_buy & df['RSI'] < 40)  # Volume trend and oversold condition
            )
        )
        
        # Enhanced sell conditions (more balanced)
        ao_sell = (df['AO'] > 0) & (df['AO'] < df['AO_MA'])  # Original AO condition
        macd_sell = df['MACD'] < df['MACD_Signal']  # MACD bearish
        rsi_sell = df['RSI'] > 70  # Overbought RSI
        
        # Sell signal requires either AO condition with confirmation or strong overbought
        sell_signal = (
            (ao_sell & (macd_sell | (df['RSI'] > 60))) |  # AO sell with confirmation
            (rsi_sell & macd_sell) |  # Strong overbought with MACD confirmation
            (df['RSI'] > 75)  # Very strong overbought
        )
